    ...
    public Mono<UserInfo> getGoogleUserInfo(final String accessToken) {
        return webClientBuilder.build()
                ...
                .uri("https://www.googleapis.com/oauth2/v3/tokeninfo",
                        uriBuilder -> uriBuilder.queryParam("accessToken", accessToken).build())
                .retrieve()
                .bodyToMono(UserInfo.class)
                .switchIfEmpty(Mono.error(new InvalidAccessTokenException(HttpStatus.BAD_REQUEST.value(), "Invalid Google AccessToken")));
    }
